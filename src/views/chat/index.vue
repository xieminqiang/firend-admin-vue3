   ...
   <div class="chat-container" ref="chat">
    <div class="out-chat-left">
      <div id="conversation" class="chat-left">
        <div v-if="conversationList.length==0" style="margin:10px">暂无会话</div>
        <div :class="item.conversationID == conversationID ?'conversation-item':'conversation-item c-active'" @click="clickConversation(item)"  @click.right="showRightMenu(item.conversationID)" v-for="item in conversationList" :key="item.conversationID">
          <el-avatar :size="'medium'" :fit="'contain'" :src="item.userProfile.avatar"></el-avatar>
          <div>
            <div class="conversation-name">
              {{ item.userProfile.nick }}
              <div v-show=" item.unreadCount > 0" class="conversation-red">{{ item.unreadCount }}</div>
            </div>
            <div class="conversation-message">{{ item.lastMessage.messageForShow }}</div>
          </div>
        </div>
      </div>
    </div>
      
    <context-menu class="context-menu-container" 
    :target="contextMenuTarget" 
    :show="contextMenuVisible" 
    @update:show="MenuUpdateShow"
   >
      <a href="javascript:;" @click="delConversation(menuCid)">删除</a>
        ...
        <div class="chat-right"> 
            <div class="chart-right"  @scroll="scrollEventSay">
              <div v-for="(item, index) in wordList" class="message-container" :key="item.id">
                <!-- 时间 -->
                ...
                <div class="word row-center" v-if="item.mineMsg == false">
                  <el-avatar :size="'medium'" :src="item.avatar ? item.avatar : avatar" alt="" />
                  <div v-if="item.cv">
                      <img class="image-element" :src="item.imageUrl" />
                    </div>
                  <div v-else class="flex-center message"
                    style="word-break: break-all">
                    <template v-for="(item, index) in decodeText(item.contactText)">
            ...
            <div v-show="conversationID != '' " class="message-input">
              ...
              <div class="send-header-bar">
                <el-popover placement="top" width="400" trigger="click">
                  ...
                  <i class="icon-smile" slot="reference" title="发表情">😀</i>
                </el-popover>
                <i class="icon-tupian" title="发图片" @click="handleSendImageClick">📷</i>
              </div>

              <div class="bottom">
                <textarea ref="text-input" rows="4" resize="false" v-model="messageContent" class="text-input" @focus="focus = true" @blur="focus = false" @keydown.enter.exact.prevent="sendText" >
                </textarea>
                <el-button slot="reference"  @click="sendText">发送</el-button>
              </div>
              <input type="file" id="imagePicker" ref="imagePicker" accept=".jpg, .jpeg, .png, .gif, .bmp" @change="sendImage" style="display: none" />
            </div>
        </div>
    </div>
...
export default {
  ...
  methods:{
    async login(){
      ...
      let promise = this.Tim.login({userID: this.currentImAccount(), userSig: this.currentImSign()});
    ...
    buildData(messageList){
      let wordList = messageList.map((item) => {
          if (item.from == this.currentImAccount() && item.type == TencentCloudChat.TYPES.MSG_TEXT) {
            return {
              mineMsg: true,
          ...
          } else if (item.from == this.currentImAccount() && item.type == TencentCloudChat.TYPES.MSG_IMAGE) {
            return {
              mineMsg: true,
              cv: true,
          ...
          } else if (item.from != this.currentImAccount() && item.type == TencentCloudChat.TYPES.MSG_TEXT) {
            return {
              mineMsg: false,
              contactText: item.payload.text,
              read: false,
    ...
    loadMoreMessage(){
      ...
      let cID = `${this.conversationID}`
      // 打开某个会话时，第一次拉取消息列表，注意！第一次拉取时不要传入 nextReqMessageID
      let promise = this.Tim.getMessageList({conversationID:cID,nextReqMessageID: this.nextReqMessageID, count: 15});
      promise.then(imResponse => {
