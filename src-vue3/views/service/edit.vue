        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="服务状态" prop="status">
              <el-radio-group v-model:value="formData.status">
...
export default {
  ...
  methods: {
    // 初始化
    async init() {
      await Promise.all([this.getCategories(), this.getServiceData()])
    },
    // 获取服务分类
    async getCategories() {
      try {
    ...
    async handleSubmit() {
      try {
        ...
        const res = await editService(this.serviceId, this.formData)
        if (res.code === 0) {
          this.$message.success(res.msg || '编辑服务成功')
          this.$router.push('/service/list')
        } else {
          this.$message.error(res.msg || '编辑服务失败')
        }
      } catch (error) {
        if (error !== false) {
          // 表单验证失败时返回false
          this.$message.error('编辑服务失败')
